...
This configuration mirrors the module-style PR in the screenshot: the root config calls `modules/repo`, and that module manages the repository plus repository rulesets for `refs/heads/main` and branches that match `refs/heads/feature*` or `refs/heads/feature/*`.
...
The feature branch ruleset applies to branch refs matching `refs/heads/feature*` and `refs/heads/feature/*`. GitHub's `*` wildcard does not cross `/`, so both patterns are needed to cover names like `feature-branch` and `feature/delete-test`. The ruleset blocks deletion but intentionally allows non-fast-forward updates so developers can rebase and push with `git push --force-with-lease`. GitHub cannot enforce "with lease" server-side; it can only allow or deny force pushes.
